...
public class ODataOpenApiMiddleware
{
    ...
    public ODataOpenApiMiddleware(IServiceProvider serviceProvider, RequestDelegate next, string requestName)
    {
        ...
        {
            this.Initialize(odataOptionsOptions.Value);
        }
    }
    ...
    public async Task Invoke(HttpContext context)
    {
        ...
        string prefixName;
        if (this.TryGetPrefixName(context, out prefixName))
        {
            await this.ProcessOpenApiAsync(context, prefixName).ConfigureAwait(false);
        }
        else
        {
    ...
    private void Initialize(ODataOptions options)
    {
        ...
        {
            string openapiPath = string.IsNullOrEmpty(model.Key) ? $"/{this._requestName}" : $"/{model.Key}/{this._requestName}";
            this.AddRoute(model.Key, openapiPath);
        }
    }
    ...
    private void AddRoute(string prefixName, string routeTemplate)
    {
        ...
        this._templateMappings[matcher] = prefixName;
    }
    ...
    private bool TryGetPrefixName(HttpContext context, out string prefixName)
    {
        ...
        foreach (var item in this._templateMappings)
        {
